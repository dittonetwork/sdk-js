...
// Определяем тип WorkflowBuilder с явными возвращаемыми типами
type WorkflowBuilder = {
	withName(name: string): WorkflowBuilder;
	withKeplerClient(client: KeplerClient): WorkflowBuilder;
	onScheduleTrigger(trigger: ScheduleTrigger): WorkflowBuilder;
	onChainEventTrigger(trigger: OnchainEventTrigger): WorkflowBuilder;
	withCount(count: number): WorkflowBuilder;
	withExpiration(timestamp: number): WorkflowBuilder;
	addJob(enrich: (jobBuilder: JobBuilder) => JobBuilder): WorkflowBuilder;
	withSigner(newSigner: WorkflowSigner): WorkflowBuilder;
	buildAndSign(): Promise<Workflow>;
};
...
function createWorkflowBuilder(): WorkflowBuilder {
	 ...
	 */
	const withName = (name: string): WorkflowBuilder => {
		if (!name || name.trim() === '') {
	 ...
	 */
	const withKeplerClient = (client: KeplerClient): WorkflowBuilder => {
		keplerClient = client;
		return builder;
	};
	 ...
	 */
	const onScheduleTrigger = (trigger: ScheduleTrigger): WorkflowBuilder => {
	 ...
	 * });
	 */
	const onChainEventTrigger = (
		trigger: OnchainEventTrigger,
	): WorkflowBuilder => {
	 ...
	 * builder.withCount(3);
	 */
	const withCount = (count: number): WorkflowBuilder => {
		state.count = count;
		return builder;
	};
	 ...
	 */
	const withExpiration = (timestamp: number): WorkflowBuilder => {
	 ...
	 */
	const addJob = (
		enrich: (jobBuilder: JobBuilder) => JobBuilder,
	): WorkflowBuilder => {
	 ...
	 */
	const withSigner = (newSigner: WorkflowSigner): WorkflowBuilder => {
		signer = newSigner;
		return builder;
	};
